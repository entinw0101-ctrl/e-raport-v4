...
export async function GET(request: NextRequest, { params }: { params: Promise<{ kelas_id: string }> }) {
  try {
    ...
    if (indikatorSikap.length === 0) {
      return NextResponse.json({ success: false, error: "Tidak ada indikator sikap" }, { status: 404 })
    }

    // Create workbook
    const workbook = new ExcelJS.Workbook()
    const worksheet = workbook.addWorksheet(`Template Penilaian Sikap - ${kelas.nama_kelas}`)

    // Define columns
    const columns = [
      { header: "NIS", key: "nis", width: 15 },
      { header: "Nama Siswa", key: "nama", width: 25 },
      { header: "Indikator Sikap", key: "indikator", width: 25 },
    ...
    siswaInKelas.forEach((siswa, studentIndex) => {
      ...
      indikatorSikap.forEach((indikator, indicatorIndex) => {
        ...
        const isLastIndicator = indicatorIndex === indikatorSikap.length - 1
      ...
      if (indikatorSikap.length > 1) {
        const startRow = currentRow - indikatorSikap.length
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename=template_penilaian_sikap_${kelas.nama_kelas}_semester_${periodeAjaran.semester === "SATU" ? "1" : "2"}_${new Date().toISOString().split("T")[0]}.xlsx`,
      },
    })
  } catch (error) {
    console.error("Error generating penilaian sikap template:", error)
    return NextResponse.json({ success: false, error: "Gagal membuat template penilaian sikap" }, { status: 500 })
  }
}