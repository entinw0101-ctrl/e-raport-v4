...
export default function PenilaianSikapPage() {
  ...
  const handleBulkDelete = async (selectedIds: (number | string)[]) => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Berhasil",
          description: `Berhasil menghapus ${selectedIds.length} data penilaian sikap`,
        })
        setSelectedIds([])
  ...
  const handleViewData = (student: any) => {
    ...
    router.push(`/penilaian-sikap/${student.id}?periode_ajaran_id=${selectedPeriodeForTable}`)
  }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <PageHeader title="Penilaian Sikap" description="Daftar siswa untuk melihat data penilaian sikap" />
