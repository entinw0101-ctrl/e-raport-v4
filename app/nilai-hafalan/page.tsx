...
export default function NilaiHafalanPage() {
  ...
  const handleBulkDelete = async (selectedIds: (number | string)[]) => {
    if (!selectedPeriodeForTable) {
      toast({
        ...
        description: "Silakan pilih periode ajaran terlebih dahulu",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      const response = await fetch("/api/nilai-hafalan/bulk-delete", {
        ...
        body: JSON.stringify({
          ids: selectedIds,
          periode_ajaran_id: selectedPeriodeForTable,
        }),
      })
      ...
      if (result.success) {
        toast({
          title: "Berhasil",
          description: `Berhasil menghapus ${selectedIds.length} data nilai hafalan`,
        })
        setSelectedIds([])
  ...
  const handleViewData = (student: any) => {
    ...
    router.push(`/nilai-hafalan/${student.id}?periode_ajaran_id=${selectedPeriodeForTable}`)
  }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <PageHeader title="Nilai Hafalan" description="Daftar siswa untuk melihat nilai hafalan" />
