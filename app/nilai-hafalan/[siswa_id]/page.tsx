...
export default function StudentNilaiHafalanPage() {
  ...
  const handleBulkDelete = async (selectedIds: (number | string)[]) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus ${selectedIds.length} data nilai hafalan yang dipilih?`)) return

    try {
      const response = await fetch("/api/nilai-hafalan/bulk-delete", {
        method: "POST",
        headers: {
      ...
      if (result.success) {
        toast({
          title: "Berhasil",
          description: `${selectedIds.length} data nilai hafalan berhasil dihapus`,
        })
        setSelectedIds([])
        fetchStudentData()
      } else {
      ...
      toast({
        title: "Error",
        description: "Gagal menghapus data nilai hafalan",
        variant: "destructive",
      })
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Predikat",
      render: (value: any) => {
        const formattedValue = value === "TIDAK_TERCAPAI" ? "Tidak Tercapai" : value
        return <Badge variant={getGradeBadgeVariant(formattedValue)}>{formattedValue}</Badge>
      },
    },
    {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <PageHeader
          title={`Nilai Hafalan - ${data.siswa.nama}`}
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p><strong>Total Nilai Hafalan:</strong> {data.nilaiHafalan.length}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <DataTable
        ...
        data={data.nilaiHafalan}
        columns={columns}
        loading={false}
        selectable={true}
        onSelectionChange={handleSelectionChange}
        onBulkDelete={handleBulkDelete}
        emptyMessage="Belum ada data nilai hafalan untuk siswa ini"
        actions={false}
        onEdit={undefined}
        onDelete={undefined}
      />
